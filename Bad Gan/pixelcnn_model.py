...
class WN_Conv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        norm_weight = self.weight * (weight_scale[:,None,None,None] / torch.sqrt((self.weight ** 2).sum(3,keepdim=True).sum(2,keepdim=True).sum(1,keepdim=True) + 1e-6)).expand(self.weight.shape)
        if old_version:
            bias = self.bias
        else:
            bias = None
